...
//		zkServer.getZookeeper().setData("/aclimooc/testdigest", "now".getBytes(), 1);
		...
		Id ipId1 = new Id("ip", "121.237.61.229");
		aclsIP.add(new ACL(Perms.ALL, ipId1));
		zkServer.createZKNode("/acl/iptest8", "iptest".getBytes(), aclsIP);

		// 验证ip是否有权限
		zkServer.getZookeeper().setData("/acl/iptest8", "now".getBytes(), 0);
		Stat stat = new Stat();
		byte[] data = zkServer.getZookeeper().getData("/acl/iptest8", false, stat);
		System.out.println(new String(data));
